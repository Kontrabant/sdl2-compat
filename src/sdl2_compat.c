...
SDL_SetRenderTarget(SDL_Renderer *renderer, SDL_Texture *texture)
{
    ...
    if (texture) {
        /* When initially setting a render target, store the main view presentation mode,
         * as it needs to be queried when transforming mouse coordinates.
         */
        SDL_Texture *current_target = SDL3_GetRenderTarget(renderer);
        if (!current_target) {
            SDL_RendererLogicalPresentation mode = SDL_LOGICAL_PRESENTATION_DISABLED;
            SDL_PropertiesID render_props = SDL3_GetRendererProperties(renderer);
            SDL3_GetRenderLogicalPresentation(renderer, NULL, NULL, &mode);
            SDL3_SetNumberProperty(render_props, PROP_RENDERER_MAIN_VIEW_LOGICAL_PRESENTATION, mode);
        }
    } else {
        SDL_PropertiesID render_props = SDL3_GetRendererProperties(renderer);
        SDL3_ClearProperty(render_props, PROP_RENDERER_MAIN_VIEW_LOGICAL_PRESENTATION);
    }
